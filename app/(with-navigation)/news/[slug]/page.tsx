...
import NewsSlugContent from "@/components/news/news-slug-content";
...
export default async function NewsPage({ params }: Props) {
  ...
  const newsArticle = await fetchArticle(slug);

  // if (!newsArticle) {
  //   notFound();
  // }

  // Split newsArticle.content into paragraphs
  const paragraphs = newsArticle.content
    .split("\n")
        ...
        <header className="mb-8">
          ...
          <div className="mb-6 flex flex-wrap items-center gap-4 border-b border-gray-300 pb-4 text-sm text-gray-600">
            <div className="flex items-center gap-1">
              ...
              <span>By {newsArticle.user_info.name}</span>
            </div>
            <div className="flex items-center gap-1">
              <Calendar className="h-4 w-4" />
              <span>
                {new Date(newsArticle.created_at).toLocaleDateString()}
              </span>
            </div>
            {/* <div className="flex items-center gap-1">
        ...
        <div className="article-content mb-12">
          ...
          <p className="mb-6 font-serif text-lg leading-relaxed">
            <span className="float-left mr-2 font-serif text-6xl font-bold leading-none">
              {newsArticle.summary.charAt(0)}
            </span>
            {newsArticle.summary.substring(1)}
          </p>

          {/* Rest of the news */}
          <NewsSlugContent slug={slug} />
        </div>
